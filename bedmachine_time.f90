PROGRAM bedmachine_time
  ...
  CHARACTER(LEN=80) :: cf_out = 'bedmachine_time.nc'
  CHARACTER(LEN=80) :: cv_lon = 'nav_lon'
  CHARACTER(LEN=80) :: cv_lat = 'nav_lat'
  CHARACTER(LEN=80) :: cv_x = 'x'
  CHARACTER(LEN=80) :: cv_y = 'y'
  ...
  IF ( narg == 0 ) THEN
     PRINT *,' usage :  bedmachine_time.exe -f BEDMACHINE-file [ -o BEDMACHINE-time]'
     PRINT *,'         -v VAR-lst -prec PREC-lst'
     PRINT *,'      '
     PRINT *,'     PURPOSE :'
     PRINT *,'        Create a BedMachine file from input file adding a UNLIMITED time'
     PRINT *,'         dimension to selected variables.'
     PRINT *,'        This program assumes that BedMachine file has already nav_lon, nav_lat'
     PRINT *,'      '
     PRINT *,'     ARGUMENTS :'
     PRINT *,'       -f BEDMACHINE-file : specify the name of the bedmachine like file.' 
     ...
     PRINT *,'     OPTIONS : '
     PRINT *,'       -o BEDMACHINE-out : name of output file'
     PRINT *,'      '
  ...
  SUBROUTINE WriteOutput ( cd_fout)
    ...
    INTEGER(KIND=4), DIMENSION(:)  , ALLOCATABLE :: ivx, ivy
    ...
    ierr=NF90_INQ_DIMID( ncidi, 'x', id) ; ierr = NF90_INQUIRE_DIMENSION(ncidi,id,len=npiglo)
    ierr=NF90_INQ_DIMID( ncidi, 'y', id) ; ierr = NF90_INQUIRE_DIMENSION(ncidi,id,len=npjglo)
    ...
    ! Variables
    ierr = NF90_DEF_VAR(ncid,'mapping' , NF90_CHAR,  (/1/) ,idmap )
    ...
    ierr = NF90_DEF_VAR(ncid,cv_x   , NF90_INT,    (/idx/)     ,idvx,  deflate_level=1,chunksizes=(/npiglo/)      )
    ierr = NF90_DEF_VAR(ncid,cv_y   , NF90_INT,    (/idy/)     ,idvy,  deflate_level=1,chunksizes=(/npjglo/)      )
...
!       ierr = NF90_PUT_ATT(ncid,idv(jvar) ,'missing_value',spval)
    ...
    !mapping
    ierr = NF90_INQ_VARID( ncidi,'mapping',id) 
    ...
    ! now write :
    ierr = NF90_PUT_VAR(ncid,idmap,'')   ! mapping
    ierr = NF90_INQ_VARID (ncidi,cv_x, id ) ; ierr = NF90_GET_VAR(ncidi,id,ivx(:) ) ; ierr = NF90_PUT_VAR(ncid,idvx,ivx)  ! x


    ierr = NF90_INQ_VARID (ncidi,cv_y, id ) ; ierr = NF90_GET_VAR(ncidi,id,ivy(:) ) ; ierr = NF90_PUT_VAR(ncid,idvy,ivy)  ! y 
    ierr = NF90_INQ_VARID (ncidi,cv_lon,id ) ; ierr = NF90_GET_VAR(ncidi,id,dltab(:,:) ) ; ierr = NF90_PUT_VAR(ncid,idlon,dltab)  ! nav_lon
    ierr = NF90_INQ_VARID (ncidi,cv_lat,id ) ; ierr = NF90_GET_VAR(ncidi,id,dltab(:,:) ) ; ierr = NF90_PUT_VAR(ncid,idlat,dltab)  ! nav_lat
